...
public class DefaultCommand implements Runnable {
    ...
    public void run() {
        ...
        try (final var input = io.openInput(jsonMapper, charset, conf.input(), conf.bufferReaderSize(), conf.autoList());
    ...
    public record Conf(
            ...
            @Property(value = "auto-list", defaultValue = "true", documentation = "If input is a XML file and some nested tag are duplicated (XML list), replace child by a list to keep a list in JSON. Note that it can lead to cases you have a list or an object (single case) so take care in your template of that detail.") boolean autoList) {
    }
}
